...
ITERATION_MANIFEST_PATH = EVIDENCE_ROOT / "ITERATION_MANIFEST.csv"
...
def verify_iteration_views(errors: list[str]) -> None:
    if not ITERATION_MANIFEST_PATH.is_file():
        fail(f"iteration manifest missing: {ITERATION_MANIFEST_PATH}", errors)
        return

    rows = read_csv(ITERATION_MANIFEST_PATH)
    if len(rows) != 200:
        fail(f"iteration manifest rows={len(rows)}, expected 200", errors)
    ...
    for operation in ("ADD", "MUL"):
        ...
        if iterations != list(range(1, 101)):
            fail(f"{operation} iteration views are not ordered 1..100", errors)
        ...
        for row in operation_rows:
            ...
            if relative.as_posix() != expected_relative.as_posix():
                fail(f"iteration view path mismatch: {relative} != {expected_relative}", errors)
                continue

            path = REPO_ROOT / relative
            if not path.is_file():
                fail(f"missing iteration view: {relative}", errors)
                continue
        ...
        actual_paths = {
            ...
            for path in (EVIDENCE_ROOT / operation / "iteration_views").glob(
                "iteration_*.txt"
            )
        }
        expected_paths = {
            f"evidence/2026-08-25/{operation}/iteration_views/"
            f"iteration_{number:03d}.txt"
            for number in range(1, 101)
        }
        if actual_paths != expected_paths:
            fail(f"{operation}: iteration view file set mismatch", errors)
        elif not any(f"{operation} iteration view" in error for error in errors):
    ...
    if row_paths != manifest_paths or len(row_paths) != 200:
        fail("iteration manifest contains duplicate or inconsistent paths", errors)
...
def main() -> int:
    ...
    print(
        "All immutable evidence, iteration views, statistics, judgement, "
